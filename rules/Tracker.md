...
## Current status
- Project state: **RENDERER EXPANSION + CONTENT SCALING (2026-06).** Re-architecture (D9–D15) is complete. M1.5R is fully done (Compare dual-lane ✅ 2026-06-22). Gate 2 fidelity reviews complete for Two Sum, Reverse Linked List, and Merge Two Sorted Lists. M1.8 exit gate **nearly complete** — hashmap ✅ (Two Sum), linkedList ✅ (Reverse Linked List), custom ✅ (Merge Two Sorted Lists), stack ✅ (Valid Parentheses, 2026-06-30), grid ✅ (Number of Islands, 2026-07-05), **queue ✅ (Rotting Oranges), tree ✅ (Binary Tree Level Order), graph ✅ (Course Schedule) — all 2026-07-05**; only **recursion** still needs a real problem. Authoring pipeline now hardened by a mechanical **liveness fidelity gate** + a **real-frame preview review** (D21, 2026-06-30). Next focus: M1.8 exit gate completion + M1.7 hardening. FastAPI backend deferred. See D16–D21.
...
| `M1.8` | **Generic renderer library** — all 8 renderer components ✅, all VisualState types ✅, stage.tsx dispatch ✅, SimulationRules audit ✅, mapping DSL (`types.ts` + `mapping.ts`) ✅; **exit gate** hashmap ✅ (Two Sum), linkedList ✅ (Reverse Linked List), custom ✅ (Merge Two Sorted Lists), stack ✅ (Valid Parentheses), grid ✅ (Number of Islands), queue ✅ (Rotting Oranges), tree ✅ (Binary Tree Level Order), graph ✅ (Course Schedule); only recursion still needs a real problem | ⏳ Engine done, 8 renderer exit gates met |
...
21. **Five traversal problems added (grid ×3 + tree + graph)** ✅ Gate 1 + Gate 3 (2026-07-05) — a "similar to Number of Islands" batch that closes the queue, tree, and graph M1.8 exit gates:
    - **Max Area of Island (#695)** — `grid` + stack DFS, tracks each component's area + running max (O(m·n)). LeetCode Ex1 = 911 steps.
    - **Rotting Oranges (#994)** — `grid` + queue multi-source BFS; grid mutation (fresh 1→rotten 2) IS the algorithm, so cells change value; queue carries the rot-minute. First strong **queue**-renderer showcase (wavefront).
    - **Flood Fill (#733)** — `grid` + stack DFS recolor; `image` mutates while a `state` matrix drives honest cell-coloring; early-return guard when new color == start color. 4-arg signature (`image, sr, sc, color`).
    - **Binary Tree Level Order (#102)** — first **tree**-renderer problem. Solution builds an explicit `{id,value,left,right}` node list from a LeetCode level-order array (nulls = gaps), then BFS; `status` array (0/1/2) drives node states, `cur_id` the cursor ring, queue aux the frontier. `resultSpec` render `tuple-list`.
    - **Course Schedule (#207)** — first **graph**-renderer problem. Kahn's topological sort (iterative, queue-driven); nodes auto-layout circularly (graph renderer's `applyLayout`), `status` colors idle/frontier/visited, cycle ⇒ leftover blocked nodes ⇒ `false`. Return value named `result` for the boolean panel. Edges static (idle) in v1.
    - **Each is single-approach** (batch tractability) so `supportsCompare` false. All authored through the staged pipeline; all deterministic gates green; liveness advisory-low (11–28/100, inherent to incremental traversals) but hard gate passed. **New authoring learning:** a legitimately-quiet degenerate preset (all-water / cycle / no-fresh) must be `isEdgeCase:true` or it trips the empty-aux liveness hard-block (`liveness.ts` line ~259 exempts edge cases). **Gate 2 formal fidelity review pending for all 5** — the tree & graph renderers especially, since this is their first real use. Live at `/problems/{max-area-of-island,rotting-oranges,flood-fill,binary-tree-level-order-traversal,course-schedule}`.
...
5. **M1.8 exit gate — only `recursion` left.** Need ≥1 real problem for the recursion renderer (e.g. merge sort, subsets/backtracking, Fibonacci memo) with Gate 1 + Gate 2. All other 9 renderers now have a real problem.
5a. **Gate 2 fidelity review — 6 grid/tree/graph problems** (`/problems/number-of-islands`, `max-area-of-island`, `rotting-oranges`, `flood-fill`, `binary-tree-level-order-traversal`, `course-schedule`) per `rules/FidelityReview.md`. Priorities: (a) **tree** (Level Order) — confirm tidy-tree layout is clean, the build phase isn't confusing, cursor ring tracks BFS; (b) **graph** (Course Schedule) — confirm circular layout is readable, the cycle case reads as "stuck", consider animating edges (currently static idle) in a v2; (c) grid flood-fills read correctly on their edge cases.
