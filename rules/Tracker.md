...
1. **Gate 2 fidelity review** for Two Sum (`/problems/two-sum`) and Reverse Linked List (`/problems/reverse-linked-list`) — per `rules/FidelityReview.md`. Step through each approach. For Reverse Linked List also address the cosmetic issues noted in item 13 above.
2. **Reverse Linked List cosmetic polish** — fix pointer lane colors for `new head`/`node` pointers in brute-stack; make head/tail pills follow actual `new_head`/`tail` vars after reversal begins; consider null-terminator indicator on the pointer box when a link is cleared. Tracked as part of Gate 2.
