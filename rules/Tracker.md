...
- **D14 — Mobile (rev. 2026-07-06).** Desktop = canonical no-scroll 5-panel layout, unchanged. Below `lg` (1024px, phones + tablets) the SAME `ProblemEngine` renders the **stacked-1a layout**: mobile top bar (back · title · ⋮ overflow **bottom sheet** with difficulty/topics/statement/approach/strategy/mode/theme) → slim mode-tab row → **pinned Stage** (fluid `clamp(11rem,30dvh,20rem)`; `flex-1` in Focus; pointer-pan + pinch-zoom, no tap-to-play) → scrollable body (narration → variables → result → call stack → code with always-on current-line explainer → complexity → notes) → pinned ControlDock (preset **bottom sheet**, touch-sized scrubber/transport, safe-area padding). No fixed pixel heights — dvh/clamp/flex only. Bottom sheets via shadcn `Sheet side="bottom"`. See [CompareAndResponsive.md](CompareAndResponsive.md) Part 2 v2.
...
5. **M1.8 exit gate completion** — need ≥1 real problem for each of the 6 remaining renderers (recursion, tree, stack, queue, grid, graph). Each needs Gate 1 + Gate 2. New problems now run through the D21 liveness gate + preview review.
