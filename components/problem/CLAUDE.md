...
Mobile (below `lg` = 1024px, D14 v2 — see CompareAndResponsive.md Part 2): SAME `ProblemEngine`, stacked-1a layout — mobile top bar (back · logo · title · ⋮ overflow bottom sheet: difficulty/topics/statement/approach/strategy/mode/theme) → **pinned Stage** (Learn/Compare `clamp(12rem,34dvh,22rem)`, Focus `flex-1`; pointer-pan + pinch-zoom, no tap-to-play, +/− zoom buttons desktop-only) → scroll body (narration → variables → result → call stack → code with always-on current-line explainer, body capped 45dvh → complexity → notes) → pinned ControlDock (preset bottom sheet `preset-sheet.tsx`, touch-sized scrubber/transport, safe-area padding). No fixed pixel heights — dvh/clamp/flex only. **Mode switching is overflow-sheet-only on mobile** (no on-screen tab row — the stage takes that space). Mobile Compare = single-lane until dual-lane Compare exists.
