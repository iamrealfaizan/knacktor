...
Mobile (below `lg` = 1024px, D14 v2 — see CompareAndResponsive.md Part 2): SAME `ProblemEngine`, stacked-1a layout — mobile top bar (back · title · ⋮ overflow bottom sheet: difficulty/topics/statement/approach/strategy/mode/theme) → `MobileModeTabs` row → **pinned Stage** (Learn/Compare `clamp(11rem,30dvh,20rem)`, Focus `flex-1`; pointer-pan + pinch-zoom, no tap-to-play) → scroll body (narration → variables → result → call stack → code with always-on current-line explainer, body capped 45dvh → complexity → notes) → pinned ControlDock (preset bottom sheet `preset-sheet.tsx`, touch-sized scrubber/transport, safe-area padding). No fixed pixel heights — dvh/clamp/flex only. Mobile Compare = single-lane until dual-lane Compare exists.
